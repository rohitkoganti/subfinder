...
class GetSubsThread(QThread):
    ...
    def _get_the_subs(self, location, query, subLinksList, subNameList, movieName ):
        ...
        if not subLinksList and not movieName:
            if location:
                print 'Trying release page..'
                url = 'https://subscene.com/subtitles/release?q=' + query.replace(" ", "+")
            else:
                print 'Trying title page..'
                url = 'https://subscene.com/subtitles/title?q=' + query.replace(" ", "+")
            try:
            ...
            if 'https://subscene.com/subtitles/title?' in soup.link.get('href').encode('ascii', 'ignore'):
                try:
